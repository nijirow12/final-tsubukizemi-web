...
import { fetchBeholdFeed, groupPostsByCountry, getAllImages, type BeholdPost } from '@/lib/behold'
...
export default function GlobeScene() {
  ...
  const [grouped, setGrouped] = useState<Record<string, BeholdPost[]>>({})
  ...
  const initScene = useCallback(() => {
    ...
    // Start closer (larger globe) and centered for GO ASIA
  ...
  const handleGoAsia = useCallback(() => {
    ...
    const startCamPos = camera.position.clone() // (0, 0, 14)
    const endCamPos = new THREE.Vector3(0, 0, 17.5) // pulled back only, camera stays centered
  ...
  // --- Fetch Behold feed when interactive ---
  ...
  useEffect(() => {
    ...
    fetchBeholdFeed()
      .then((feed) => {
        if (feed?.posts) {
          setGrouped(groupPostsByCountry(feed.posts))
        }
      })
      .finally(() => setIgLoading(false))
  }, [phase])

  // Get all images for selected pin (carousel expanded)
  const pinPhotos: BeholdPost[] = selectedPin ? (grouped[selectedPin] || []) : []
  const pinImages = pinPhotos.flatMap((post) => getAllImages(post))
  ...
  return (
    <div className="relative w-full h-screen bg-white overflow-hidden">
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-40 bg-[#f8fafc]/85 border-b border-[#e2e8f0]/75 backdrop-blur-[12px] transition-opacity duration-[2000ms] ease-out ${headerVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
        <div className="w-full px-[clamp(1.6rem,3.5vw,3rem)] pr-[clamp(2.8rem,5vw,5rem)] py-2.5 flex items-center justify-between gap-8">
          <a href="/home" className="no-underline text-inherit flex flex-col gap-1">
            <span className="text-[1.35rem] font-semibold tracking-[0.4em] text-[#0f172a]">
              TSUBUKI SEMINAR
            </span>
      ...
      {phase === 'idle' && (
      <div
        className="absolute inset-0 top-14 flex flex-col items-center justify-center z-10 pointer-events-none gap-8"
      ...
      >
        ...
        <button
          ...
          className="pointer-events-auto text-[clamp(2.8rem,10vw,4.5rem)] font-bold tracking-[0.22em] uppercase text-[#f8fafc] bg-transparent border-none cursor-pointer px-5 py-2 transition-all duration-300 hover:tracking-[0.28em] hover:[text-shadow:0_0_80px_rgba(41,151,255,0.5)] active:translate-y-px"
          style={{
      ...
      {selectedPin && (
        <>
          ...
          {/* Scattered photos around the globe */}
          {(() => {
            const positions = [
            ...
            return pinImages.slice(0, 12).map((img, i) => {
              const pos = positions[i % positions.length]
              return (
                <a
                  key={`burst-${img.permalink}-${i}`}
                  href={img.permalink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="absolute z-20 rounded-xl overflow-hidden shadow-[0_8px_30px_rgba(15,23,42,0.15)] border-2 border-white hover:scale-110 hover:z-30 hover:shadow-[0_16px_40px_rgba(15,23,42,0.25)] transition-all duration-300"
                  style={{
                    top: pos.top,
